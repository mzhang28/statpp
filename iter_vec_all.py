...
for outer in pbar:
    with torch.no_grad():
        ...
        # per-play sigma weights, normalized to mean 1
        play_w = map_sigma[m_t]
        play_w = play_w / play_w.mean()

    # D: weighted SGD refit on log-survivals
    ...
    pbar.set_postfix_str(
        f"d={delta:.2f} mse={final_mse:.4f} σ={sigma_np.mean():.3f}±{sigma_np.std():.3f} | {' '.join(known_str)}"
    )

print_rankings("final")

np.savez_compressed(
    f"{OUTPUT}/itersvd_all.npz",
